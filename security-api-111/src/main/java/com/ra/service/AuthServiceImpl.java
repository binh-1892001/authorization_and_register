...
public class AuthServiceImpl implements AuthService
{
    ...
    @Override
    public UserLoginResponseDTO login(UserLoginRequestDTO userLoginRequestDTO) throws CustomException
    {
    ...
    public void register(FormRegister formRegister)
    {
        ...
        // thực hiện tìm ra đối tượng role
        roles.add(findByRoleName("ROLE_USER"));


        // chuyển đổi từ formRegister => User
        User user = User.builder()
    ...
    @Override
    public void handleChangePassword(ChangePassword changePassword) throws CustomException
    {
        UserPrinciple userPrinciple = (UserPrinciple) SecurityContextHolder.getContext().getAuthentication().getPrincipal();

        // oldPassword khi gửi lên phải giống mật khẩu trong database
        if (passwordEncoder.matches(changePassword.getOldPassword(), userPrinciple.getUser().getPassword()))
        {
            ...
            {
                throw new CustomException("repassword not match");
            }
        }
        else
        {
